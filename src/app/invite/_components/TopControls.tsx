...
function MusicControl() {
    ...
    return (
        <div className="pointer-events-auto">
            <Button onClick={toggleMute} aria-label={muted ? 'Unmute music' : 'Mute music'} variant="ghost" size="icon">
                {muted ? <VolumeX className="size-5 text-zinc-600" /> : <Volume2 className="size-5 text-zinc-600" />}
            </Button>
            <audio ref={audioRef} src={WEDDING_EVENT.musicUrl} loop onError={handleAudioError} className="hidden" />
        </div>
    );
}
